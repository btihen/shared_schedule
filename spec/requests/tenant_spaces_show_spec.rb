...
RSpec.describe "Spaces Show", type: :request do
  ...
  let(:middle_event)      { event = FactoryBot.create :event, reason: reason2, tenant: tenant
                            event.event_space_reservations << EventSpaceReservation.create(date: middle_this_month - 1.day, space: space1, time_slot: evening)
                            event.event_space_reservations << EventSpaceReservation.create(date: middle_this_month,         space: space1, time_slot: evening)
                            event.event_space_reservations << EventSpaceReservation.create(date: middle_this_month + 1.day, space: space1, time_slot: morning)
                            event.event_space_reservations << EventSpaceReservation.create(date: middle_this_month + 1.day, space: space1, time_slot: afternoon)
                            event.event_space_reservations << EventSpaceReservation.create(date: middle_this_month + 1.day, space: space1, time_slot: evening)
                            event.save
                            event.reload
                          }
