class Managers::SpaceView < ViewObject
  ...
  def tenant
    Managers::TenantView.new(space.tenant)
  end
  ...
  def events
    Managers::EventView.collection(space.events)
  end

  def allowed_time_slots
    Managers::TimeSlotView.collection(space.allowed_time_slots)
  end

  def reservations(date_range=nil)
    return Managers::ReservationView.collection(space.reservations) if date_range.nil?

    Managers::ReservationView.collection( space.reservations.in_date_range(date_range) )
  end
