class Guests::SpaceView < ViewObject
  ...
  def tenant
    Guests::TenantView.new(space.tenant)
  end
  ...
  def events
    Guests::EventView.collection(space.events)
  end

  def allowed_time_slots
    Guests::TimeSlotView.collection(space.allowed_time_slots)
  end

  def reservations(date_range=nil)
    return Guests::ReservationView.collection(space.reservations) if date_range.nil?

    Guests::ReservationView.collection( space.reservations.in_date_range(date_range) )
  end
