class Guests::EventView < ViewObject
  ...
  def tenant
    @tenant   ||= Guests::TenantView.new(event.tenant)
  end

  def category
    @category ||= Guests::CategoryView.new(event.category)
  end

  def spaces
    @spaces   ||= Guests::SpaceView.collection(event.spaces).uniq
  end

  def reserved_time_slots
    @reserved_time_slots ||= Guests::TimeSlotView.collection(event.reserved_time_slots)
  end

  def event_reservations
    @event_reservations  ||= Guests::EventReservation.collection(event.reservations)
  end
